...
export abstract class JournalSyncAbstract {
    ...
    // async getDownloadFileInfo(name: string): Promise<DownloadedFile | false> {
    //     const kv = this.env.simpleStore as SimpleStore<DownloadedFile>;
    //     const key = `${this.hash}-downloaded-${name}`;
    //     return await kv.get(key) || false
    // }
    // async listDownloadFileInfo(): Promise<string[]> {
    //     const kv = this.env.simpleStore as SimpleStore<DownloadedFile>;
    //     const FromKey = `${this.hash}-downloaded-`;
    //     const ToKey = `${this.hash}-downloaded.`;

    //     return await kv.keys(FromKey, ToKey);
    // }
    // async setDownloadedFileInfo(param: DownloadedFile) {
    //     const kv = this.env.simpleStore as SimpleStore<DownloadedFile>;
    //     const key = `${this.hash}-downloaded-${param.name}`;
    //     await kv.set(key, { ...param })
    //     return param;
    // }
    // async downloadQueuedFile(doc: DownloadedFile) {
    //     return await this.lockAndProcess(doc.name, async () => {
    //         if (!doc.isStub) {
    //             Logger(`Already Downloaded: ${doc.name}`, LOG_LEVEL_VERBOSE)
    //             return;
    //         }
    //         const blob = await this.downloadFile(doc.name);
    //         return await this.setDownloadedFileInfo({ name: doc.name, isStub: false, isFinished: false, isErrored: blob === false, data: blob === false ? undefined : blob })
    //     });
    // }

    // async decryptDownloadedQueue(doc: DownloadedFile) {
    //     return await this.lockAndProcess(doc.name, async () => {
    //         return await this.setDownloadedFileInfo({ name: doc.name, isStub: false, isFinished: false, data: undefined, decrypted: doc.data, isErrored: false })
    //     });
    // }
    // async processDownloadedQueue(doc: DownloadedFile) {
    //     return await this.lockAndProcess(doc.name, async () => {
    //         const applyBuf = [] as QueuedDocument[]
    //         const ab = await wrappedInflate(doc.decrypted, { consume: true });
    ...
    //                 break
    //             }
    //             const data = JSON.parse(d.decode(ab.slice(idxFrom, idxTo))) as QueuedDocument;
    //             applyBuf.push(data);
    //             idxFrom = idxTo + 1
    //         } while (idxTo > 0);
    //         return await this.setDownloadedFileInfo({ name: doc.name, isStub: false, isFinished: false, isErrored: false, processDocuments: applyBuf })
    //     });
    // }

    // async processQueuedDocument(doc: DownloadedFile) {
    //     return await this.lockAndProcess(doc.name, async () => {
    //         if (!doc.isStub && !doc.isErrored && !doc.isFinished && doc.processDocuments) {
    //             const docs = doc.processDocuments;
    //             try {
    //                 // Logger(`Processing ${docs.length} docs`, LOG_LEVEL_VERBOSE);
    ...
    //                 const saveDocs = docs.filter(e => (e._id in diffRevs && "missing" in diffRevs[e._id] && diffRevs[e._id].missing.length > 0));
    ...
    //                 }));
    //                 await this.updateCheckPointInfo((info) => ({ ...info, }))
    //                 Logger(`Applied ${saveDocs.length} of ${docs.length}`, LOG_LEVEL_VERBOSE);
    //                 await this.setDownloadedFileInfo({ name: doc.name, isStub: false, isFinished: true, isErrored: false, processDocuments: undefined })
    //             } catch (ex) {
    //                 Logger(`Applying journal failed`, LOG_LEVEL_NOTICE);
    //                 await this.setDownloadedFileInfo({ name: doc.name, isStub: false, isFinished: true, isErrored: true, processDocuments: undefined })
    //                 Logger(ex, LOG_LEVEL_VERBOSE);
    //             }
    //         }
    //     });
    ...
    // async receiveRemoteJournal_(showMessage = false) {
    //     this.updateInfo({ syncStatus: "JOURNAL_RECEIVE" })
    //     // const info = await this.getCheckpointInfo();
    //     // const knownFiles = new Set(info.knownFiles);
    //     {
    //         // Add All to Stub
    //         const keys = await this._getRemoteJournals();
    //         const setStubTask = keys.map((name) => this.setDownloadedFileInfo({ name, isStub: true }));
    //         await Promise.all(setStubTask);
    //     }
    //     {
    //         // Process all queued Files Download
    //         const list = await this.listDownloadFileInfo();
    //         const tasks = [] as (() => Promise<void>)[];
    //         for (const name of list) {
    //             if (this.processing.has(name)) {
    //                 continue;
    //             }
    //             const doc = await this.getDownloadFileInfo(name);
    //             if (doc === false) {
    //                 Logger(`Something got wrong on: ${name}`, LOG_LEVEL_VERBOSE)
    //                 continue;
    //             }
    //             if (isFinished(doc)) continue;
    //             if (isErrored(doc)) continue;
    //             if (isStub(doc)) tasks.push(() => this.downloadQueuedFile(doc));
    //             if (isDownloaded(doc)) tasks.push(() => this.decryptDownloadedQueue(doc));
    //             if (isDecrypted(doc)) tasks.push(() => this.processDownloadedQueue(doc));
    //         }

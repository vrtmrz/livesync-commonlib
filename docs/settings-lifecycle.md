...
Object Storage and WebDAV Journal profiles without the new protocol fields retain the Opaque format and complete-pack retrieval. Adaptive Journal is never enabled as a new-Vault recommendation or an upgrade side effect. A host must record the user's explicit format and retrieval choice.

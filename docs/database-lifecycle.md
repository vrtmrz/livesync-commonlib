...
| `DatabaseEventService.onResetDatabase(db)`                  | Before destruction of the active physical database                                                                     | A `false` result prevents physical destruction. This legacy handler sequence is not transactional: an earlier handler may already have released or reset dependent state, so a failed reset requires a controlled restart. |

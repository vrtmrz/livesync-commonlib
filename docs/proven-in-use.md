...
The focused `/adaptive-journal` entry is established by Commonlib-owned deterministic fixtures and unit tests. They cover repository manifests, identity generation before creation, local identity binding, immutable record encodings, Commit Bundles, inline and external Packs, derived Catalogues, native Chunk batches, writer and Commit discovery, publication recovery after ambiguous writes, receive frontiers, and rejection of inconsistent records. The maintained Journal composition also reports an accepted identity to the profile boundary, where focused tests verify idempotent write-back into the active profile. Request-boundary tests verify that a small object-profile publication creates one Bundle, a representative external publication creates one Pack followed by one Bundle, an oversized publication partitions across Packs, and one received Bundle supplies its Commit, Metadata, and inline Pack without repeated object reads.
...
The PostgREST delivery stage adds focused tests for client-safe credentials, RPC scoping, binary fidelity, native multi-key Chunk batches, bounded PUT subdivision, oversized-GET subdivision, server-authoritative successful writes, ambiguous-write classification, Writer and Commit discovery, format detection, and remote Rebuild. Its disposable PostgreSQL and PostgREST integration applies the distributable SQL contract to a new database and exercises Vault authentication and valid-credential isolation, exact binary Chunk storage, immutable and concurrent insert conflicts, malformed and oversized batch rejection, missing-Chunk Commit rollback, immutable Writer registration, transactional Commit Bundle insertion, one-read Commit and Metadata delivery, and invalid-credential rejection through real HTTP. It does not yet establish a maintained-host CLI or UI composition.

This establishes Commonlib protocol and adapter behaviour plus maintained CLI composition for WebDAV. PostgREST CLI, Host UI, and real-Obsidian end-to-end validation remain separate delivery stages. The entries therefore remain experimental even though their v1 invariants, browser-safe configuration boundary, exact package exports, and current adapter transfer paths are checked.

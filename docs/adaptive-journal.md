...
The default inline Pack limit is 8 MiB, the writer's preferred Pack target is 32 MiB, and the wire-format ceiling is 256 MiB. The writer partitions encoded Chunk frames deterministically when adding another frame would cross the preferred target. One frame may exceed the target, but it must remain within the wire-format ceiling. A publication whose newly required Chunks fit one Pack at or below the inline limit creates one Commit Bundle. Otherwise, it creates the required external Packs first, then creates the Commit Bundle. Chunk encoding and Pack verification are serial, and external Packs are built and published one at a time with an event-loop yield between Packs. This bounds the largest additional Pack buffer and synchronous copy on memory-constrained mobile hosts, although the host still owns its complete batch and source-data memory policy. Metadata-only publication and publication which reuses already catalogued Chunks also create only the Bundle. The exact Bundle bytes are staged in durable writer state before the final create, so recovery republishes the same immutable value. A failed multi-Pack publication can leave unreferenced immutable Packs, but no partial Commit becomes visible.
...
The experimental WebDAV adapter applies the same Opaque and Adaptive Journal formats to a dedicated DAV collection. The focused `/journal-storage` entry exports `REMOTE_WEBDAV`, `WebDAVSyncSetting`, profile-field parsing and serialisation, and `isJournalStorageConnectionInspector`, while `/remote-configurations` serialises and activates `sls+webdav://` profiles. A Journal replicator's typed inspection reports the required Adaptive semantics and optional Range result separately. Whole-Pack remains available when only Range is unsupported, while a profile which selects Range fails its connection gate. Endpoint selection, credential acquisition, persistence, and presentation remain host responsibilities.
...
For example, one batch containing about 100 MiB of newly required encoded Chunk frames normally creates four external Packs at the 32 MiB target, for five writes including its Bundle. A `whole-pack` receiver which needs all of them adds four Pack reads; a `range` receiver remains governed by the number of missing Chunks. Splitting does not duplicate the Chunk frame bytes, but every additional Pack adds one object request and one authenticated route descriptor.
